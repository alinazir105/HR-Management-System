    ...
    const displayArray = (arr) => {
        return arr && arr.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">
                {arr.map((item, idx) => (
                    <Badge key={idx} className="rounded-full bg-neutral-100 border border-neutral-300 px-3 py-1 text-xs text-gray-700">{item}</Badge>
                ))}
            </div>
        ) : (
        ...
        <Drawer>
            ...
            <DrawerContent className="w-full max-w-3xl mx-auto rounded-t-xl shadow-lg pb-4">
                <DrawerHeader className="pb-2 border-b">
                    <DrawerTitle className="text-xl font-bold tracking-tight">Performance Review</DrawerTitle>
                    <DrawerDescription>
                ...
                <div className="px-6 py-4 grid sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <Badge
                            className={`px-2 py-1 ml-2 text-xs rounded-full ${review?.status === "reviewed"
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">{formatDate(review?.reviewed_at)}</p>
                    </div>
                    <div>
                ...
                <div className="px-6 pt-2 space-y-3 text-sm border-b pb-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">{review?.feedback || "No feedback given."}</p>
                    </div>
                    <div>
                        <span className="font-medium text-foreground">Goals Set</span>
                        <div className="mt-1">{displayArray(review?.goals_set || [])}</div>
                    </div>
                    <div>
                        <span className="font-medium text-foreground">Goals Achieved</span>
                        <div className="mt-1">{displayArray(review?.goals_achieved || [])}</div>
                    </div>
                    <div>
                        <span className="font-medium text-foreground">Areas to Improve</span>
                        <p className="text-muted-foreground mt-1">{review?.areas_to_improve.length == 0 ? "None mentioned." : review?.areas_to_improve}</p>
                    </div>
                </div>
